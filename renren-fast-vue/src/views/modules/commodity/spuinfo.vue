...
export default {
  ...
  methods: {
    productUp(id) {
      this.$http({
        //url: "http://localhost:9090/commodity/spuinfo/" + id + "/up",
    ...
    productDown(id) {
      this.$http({
        //url: "http://localhost:9090/commodity/spuinfo/" + id + "/down",
        url: this.$http.adornUrl("/commodity/spuinfo/" + id + "/down"),
        method: "post"
      }).then(({ data }) => {
        if (data && data.code === 0) {
          this.$message({
    ...
    getDataList() {
      ...
      this.$http({
        //url: "http://localhost:9090/commodity/spuinfo/list",
